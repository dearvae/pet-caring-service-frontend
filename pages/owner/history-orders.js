...
export default function HistoryOrders() {
  const showingSuccessfulBidsText = "Currently showing successful bids (Click to toggle)";
  const showingUnsuccessfulBidsText = "Currently showing unsuccessful bids (Click to toggle)";
  ...
  const [buttonText, setButtonText] = useState(showingSuccessfulBidsText);

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_PATH}/owners/bids/${localStorage.getItem('username')}/`)
    .then(res => res.json())
    .then(json => {
      const successful = json.filter(row => row.is_successful);
      const unsuccessful = json.filter(row => !row.is_successful);
      setSuccessfulBids(successful);
      setUnsuccessfulBids(unsuccessful);
      setDisplayRows(showSuccessfulBids ? successful : unsuccessful);
    })
  }, []);
  ...
  const toggleButtons = () => {
    ...
    setButtonText(temp ? showingSuccessfulBidsText : showingUnsuccessfulBidsText);
  }
